...
function parseArgs(argv) {
    ...
    if (multi) {
        ...
        return { mode: "multi-city", legs, options, showAll, json, select };
    }
    ...
    return {
        mode: returnDate ? "round-trip" : "one-way",
...
function printResult(result, showAll) {
    if (result.error) {
...
function printMultiCityResult(result, showAll) {
    if (result.error) {
        console.error(`Error: ${result.error}`);
        process.exit(1);
    }
 ...
 */
async function runStepSelect(legs, selections, options) {
