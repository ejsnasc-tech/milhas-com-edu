import { buildOneWayPayload, buildRoundTripPayload, buildMultiCityPayload, buildMultiCityStepPayload } from "./payload.js";
...
function bestItinerary(itineraries) {
    if (itineraries.length === 0)
        return null;
    return itineraries.reduce((best, curr) => curr.price < best.price ? curr : best);
}
/**
 * Query a one-way flight.
 */
export async function queryOneWay(origin, destination, date, options = {}) {
 ...
 */
export async function queryRoundTrip(origin, destination, departureDate, returnDate, options = {}) {
 ...
 */
export async function queryMultiCityLegacy(legs, options = {}) {
 ...
 */
function extractSelectedFlightInfo(itinerary, leg) {
 ...
 */
export async function queryMultiCity(legs, options = {}) {
    if (legs.length < 2) {
 ...
 *   ...
 */
export function createMultiCityStepper(legs, options = {}) {
    let step = 0;
    let lastItineraries = [];
    const selectedFlights = [];
    const segments = [];
 ...
 */
export async function queryBatch(queries, delay = 1500) {
    const results = [];
