import { SEAT_MAP, STOPS_MAP, TRIP_MAP } from "./types.js";
/** Build a flight segment for the request payload. */
function buildSegment(origin, destination, date, options = {}) {
...
export function buildPayload(segments, tripType = "round-trip", options = {}) {
 ...
 */
export function buildOneWayPayload(origin, destination, date, options = {}) {
 ...
 */
export function buildRoundTripPayload(origin, destination, departureDate, returnDate, options = {}) {
 ...
 */
export function buildMultiCityPayload(legs, options = {}) {
 ...
 */
function buildSegmentWithSelection(origin, destination, date, selectedFlight, options = {}) {
 ...
 */
export function buildMultiCityStepPayload(legs, bookingToken, selectedFlights, options = {}) {
    const seatClass = options.seatClass ?? "economy";
    const adults = options.adults ?? 1;
    const children = options.children ?? 0;
    const maxPrice = options.maxPrice != null ? [null, options.maxPrice] : null;
    ...
    const segments = legs.map((leg, i) => {
        if (i < selectedFlights.length) {
            return buildSegmentWithSelection(leg.origin, leg.destination, leg.date, selectedFlights[i], options);
        }
