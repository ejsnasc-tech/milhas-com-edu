...
function makeItinerary(price, legs) {
    return [
...
function makeResponse(itineraries, blockIndex = 2) {
    const inner = new Array(4).fill(null);
...
describe("parseResponse", () => {
    ...
    it("skips itineraries without a price", () => {
        ...
        const text = makeResponse([noPriceItin, validItin]);
        const result = parseResponse(text);
        expect(result).toHaveLength(1);
        expect(result[0].price).toBe(100);
    });
    ...
    it("handles legs with missing optional fields", () => {
        const shortLeg = new Array(4).fill(null);
