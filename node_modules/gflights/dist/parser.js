 ...
 */
function parseItinerary(k) {
    try {
        const priceArr = k[1];
        const priceInner = priceArr?.[0];
        const price = priceInner?.[1];
        if (price == null)
            return null;
        const legs = [];
        let totalDuration = 0;
        const flightData = k[0];
        const legsList = flightData?.[2];
...
export function parseResponse(text) {
    ...
    for (const blockIdx of [2, 3]) {
        try {
            const block = inner[blockIdx];
            if (!block?.[0])
                continue;
            for (const entry of block[0]) {
